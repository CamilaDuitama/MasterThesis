...
```{r Percentage of Ids lost per gene set in training}
...
for (i in unique(db_genesets$gs_name)){
  ...
  lp.new[[i]] <- predict(object =reg.pcox[[i]], newx = as.matrix(df.new), s=min(reg.pcox[[i]]$lambda) ,type="link")
}

```

### Percentage of ids lost per gene set in training set
```{r Percentage of Ids lost per gene set in Test set}
...
colnames(Y.pre.test)<-names(lp.new)
```
...
### Headers of matrixes with linear predictors per gene set
...
p8 <- ggplot(as.data.frame(pc.unscaled$x), aes(x=pc.unscaled$x[,1], y= pc.unscaled$x[,2], colour= as.factor(labels))) + ggtitle("PCA \n Training set with SBC labels and unlabeled test set") + geom_point(shape=19) + labs(y = "PC1", x = "PC2", colour = "Classes")+scale_color_manual(values=c("#b16ed3","#d37f6e","#90d36e","#6ec2d3","#C70039"))
p8
```
