...
```{r Preparation of Data Structures to train the SBC, eval=FALSE, include=FALSE}
########################################################################
...
p5 <- ggplot(as.data.frame(pc.pred), aes(x=pc.pred[,1], y= pc.pred[,2], colour= as.factor(c.final))) + ggtitle(" SBC Clustering \n TCGA-GBM Training Set") + geom_point(shape=19) + labs(y = "PC1", x = "PC2", colour = "Classes") +scale_color_manual(values=c("#b16ed3","#d37f6e","#90d36e","#6ec2d3"))
p5
```
...
p4.new <- ggplot(as.data.frame(pc.pred), aes(x=pc.pred[,1], y= pc.pred[,2], colour= as.factor(c.sbc.new))) + ggtitle(" SBC Clustering \n TCGA-GBM Test Set") + geom_point(shape=19) + labs(y = "PC1", x = "PC2", colour = "Classes")+scale_color_manual(values=c("#b16ed3","#d37f6e","#90d36e","#6ec2d3"))
p4.new
```
...
Methods<-c("Flexmix","K-Means + Penalized Cox","SBC + AFT","SBC")
recovCIndex<-round(c(recovCIndex.flx,recovCIndex.km.pcox,max(recovCIndex.sbc.paft),max(recovCIndex.sbc)),digits = 3)
predCIndex<-round(c(predCIndex.flx,predCIndex.kk.pcox,predCIndex.sbc.aft,max(predCIndex.sbc)),digits=3)
