...
c.matrix.new.sbc <- matrix(0, nrow = N.new, ncol = Nps)
...
lr <- c(0)
for (j in 1:Nps.mod){
  lr[j] <-  1 - pchisq(unlist(survdiff(smod.new ~ c.matrix.new.sbc[,j]))$chisq,df = nlevels(as.factor(c.matrix.new.sbc[,j])) -1 )
