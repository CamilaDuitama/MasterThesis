...
```{r Download NCI Pathways, eval=FALSE, include=FALSE}
...
#PLAGE
Y.pre.train.plage <- t(gsva(t(Y.pre.train),pathwaysets,mx.diff=TRUE,verbose=FALSE,parallel.sz=1,method="plage"))
Y.pre.test.plage <-t(gsva(t(Y.pre.test), pathwaysets, mx.diff=TRUE, verbose=FALSE, parallel.sz=1,method="plage"))
...
for (i in unique(db_genesets$gs_name)){
  ...
  reg.paft.new[[i]] <- glmnet(x = as.matrix(df.new), y = time.new, family = "gaussian")
  linear.aft.new[[i]] <- predict(object = reg.paft.new[[i]], newx = as.matrix(df.new), s = min(reg.paft.new[[i]]$lambda)) 
