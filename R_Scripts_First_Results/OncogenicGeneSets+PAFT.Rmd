...
params:
  ...
  iter.thin: 5
  pval.sbc : 0.95
  output_folder : "OcogenicGeneSets+PAFT_files/"
...
```{r GeneSet PCox fitting on training set, eval=FALSE, include=FALSE}
gs<-list()
reg.paft<-list()
linear.aft<-list()
lost<-list()
...
## Test set
```{r Gene Set Pcox fitting on test set, eval=FALSE, include=FALSE}
gs<-list()
reg.paft.new<-list()
linear.aft.new<-list()
lost.new<-list()
...
for (i in unique(db_genesets$gs_name)){
  ...
  linear.aft.new[[i]] <- predict(object = reg.paft[[i]], newx = as.matrix(df.new), s = min(reg.paft[[i]]$lambda)) 
...
```{r Momentum matching, eval=FALSE}
...
# Y.pre.train <- scale(Y.pre.train, center = TRUE, scale = TRUE)
# Y.pre.test<- scale(Y.pre.test, center=TRUE,scale=TRUE)
# # 
...
#  }
# # ##Momentum matching##
#  Y.pre.test.prime<-momentmatching(Y.pre.test,Y.pre.train)
#  Y.pre.test.prime<-scale(Y.pre.test.prime, center=TRUE,scale=TRUE)
#  obj <- scale(Y.pre.train, center = TRUE, scale = TRUE)
#  Y.pre.test.prime.scaled<-  scale(Y.pre.test.prime,center = attr(obj,"scaled:center"), scale = (attr(obj,"scaled:scale")))
# 
# ##Scaling##
# 
...
Y.big.corrected<-rbind(Y.pre.train,Y.pre.test)
Y.big.corrected<-scale(Y.big.corrected,scale=TRUE,center=TRUE)
pc <- prcomp(Y.big.corrected,center=TRUE,scale = TRUE)
labels<-c.sbc
for (i in 1:dim(Y.pre.test)[1]){labels<-append(labels,5)}
...
```{r PCA , echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
Y.pre.train<-do.call(cbind,linear.aft)
colnames(Y.pre.train)<-names(linear.aft)
...
#obj <- scale(Y.pre.train, center = TRUE, scale = TRUE)
#Y.pre.test <- scale(Y.pre.test, center = attr(obj,"scaled:center"), scale = (attr(obj,"scaled:scale")))
...
p5 <- ggplot(as.data.frame(pc.pred), aes(x=pc.pred[,1], y= pc.pred[,2], colour= as.factor(c.final))) + ggtitle(" SBC Clustering \n VerhaakCancer Data Set") + geom_point(shape=19) + labs(y = "PC1", x = "PC2", colour = "Classes")+scale_color_manual(values=c("#b16ed3","#d37f6e","#90d36e","#6ec2d3","#C70039"))
p5
```
...
p8 <- ggplot(as.data.frame(pc.unscaled$x), aes(x=pc.unscaled$x[,1], y= pc.unscaled$x[,2], colour= as.factor(labels))) + ggtitle("Scaled data") + geom_point(shape=19) + labs(y = "PC1", x = "PC2", colour = "Classes")+scale_color_manual(values=c("#b16ed3","#d37f6e","#90d36e","#6ec2d3","#D4D4D4"))
p8
```
...
p4.new <- ggplot(as.data.frame(pc.pred), aes(x=pc.pred[,1], y= pc.pred[,2], colour= as.factor(c.sbc.new))) + ggtitle(" SBC Clustering \n GBM Cancer Test Data Set") + geom_point(shape=19) + labs(y = "PC1", x = "PC2", colour = "Classes") +scale_color_manual(values=c("#b16ed3","#d37f6e","#90d36e","#6ec2d3"))
p4.new
```
