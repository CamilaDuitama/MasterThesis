...
CN_pval <- colnames(CopyNumber_TrainingSet)[(table.adj < 0.00000005)] 
...
CN_pval.new <- colnames(CopyNumber_TestSet)[(table.adj.new < 0.00000005)] 
...
Cluster2vsCluster4=cluster2 - cluster4,Cluster3vsCluster4=cluster3 - cluster4,levels= mm)
###### Fit limma model
fit <- lmFit(t(miRNA_TrainingSet), design = mm)
efit <- eBayes(fit)
###### Look for DE table
topmiRNA.train<-rownames(topTable(efit,number=50,adjust.method="BH"))
...
Cluster2vsCluster4=cluster2 - cluster4,Cluster3vsCluster4=cluster3 - cluster4,levels= mm)
###### Fit limma model
fit.new <- lmFit(t(miRNA_TestSet), design = mm.new)
efit.new <- eBayes(fit.new)
###### Look for DE table
topmiRNA.test<-rownames(topTable(efit.new,number=50,adjust.method="BH"))
...
ordered_labels.new<-miRNA.matrix.test$sbc.labels
miRNA.matrix.test$sbc.labels<-NULL

hmcols<-colorRampPalette(c("green","black","red"))(256)
heatmap.2(x = t(miRNA.matrix.test),Colv = FALSE, Rowv=TRUE, dendrogram= c("row"), scale ="row",ColSideColors =c("green","red","blue","orange")[ordered_labels.new], labRow = colnames(miRNA.matrix.test), labCol = NA, main = ' \n Test Set \n Differentially expressed miRNA transcripts', col =hmcols, cexRow =0.40, trace ="none", key.title = "Color Key")
legend("topright", legend = c("Best","Good Moderate","Bad moderate","Worst"),fill = c("Green","Blue","Orange","Red"), cex = 0.4)
```
