...
Comparisonx = function(){
  ...
  reg.pcox <- glmnet(x = Y, y = smod, family = "cox")
  lp <- predict(object =reg.pcox, newx = Y, s= reg.pcox$lambda[35],type="link")
  recovCIndex.na.cox <<- as.numeric(concordance(smod ~exp(-lp))[1])
  
  ######## Prediction with penalized Cox Proportional Hazards Model ###########################################
  linear.pred.cox <- c(0)
  ### see if we can use glmnet
  reg.pcox <- glmnet(x = Y, y = Surv(exp(time), censoring), family = "cox")
  linear.pred.cox <- predict(object =reg.pcox, newx = as.matrix(Y.new), s=reg.pcox$lambda[35])
  predCIndex.na.cox <<- as.numeric(concordance(smod.new ~ exp(-linear.pred.cox))[1])
  
  
  #### Fitting a penalized AFT Model ####
  reg <- glmnet(x = Y, y = time, family = "gaussian")
  linear.aft <- predict(object = reg, newx = Y, s = reg.pcox$lambda[35]) 
  cindex.paft <- as.numeric(concordance(smod ~ linear.aft)[1])
  recovCIndex.na.aft <<- as.numeric(cindex.paft)
  ##### Prediction using the penlized AFT #############################################
  linear.pred.paft <- c(0)
  ### see if we can use glmnet
  reg.paft <- glmnet(x = Y, y = time, family = "gaussian")
  linear.pred.paft <- predict(object = reg.paft, newx = as.matrix(Y.new), s= reg.pcox$lambda[35])
  predCIndex.na.aft <<- as.numeric(concordance(smod.new ~ linear.pred.paft)[1])
  ...
  for ( q in 1:F){
    ...
    reg.pcox <- glmnet(x = Y.tmp, y = Surv(coxreg$time, coxreg$status), family = "cox")
    linear.cox[ind] <- predict(object =reg.pcox, newx = Y.tmp, s= reg.pcox$lambda[35])
  }
  
  recovCIndex.km.pcox <<- as.numeric(concordance(smod ~ exp(-linear.cox))[1])
  ### Prediction with k-means + k-nearest neghbour
  linear.kkpcox.prediction <- c(0)
  for ( q in 1:F){
    ind <- which(label.train == q)
    ind.new <- which(label.test == q)
    reg.aft <- glmnet(x = Y[ind,], y = Surv(exp(time[ind]),censoring[ind]), family = "cox")
    if (length(ind.new)==1){
      linear.kkpcox.prediction[ind.new] <- predict(object =reg.aft, newx = t(as.matrix(Y.new[ind.new,])), s= reg.pcox$lambda[35])
    }else{
      linear.kkpcox.prediction[ind.new] <- predict(object =reg.aft, newx = as.matrix(Y.new[ind.new,]), s= reg.pcox$lambda[35])
      
    }
  }
  predCIndex.kk.pcox <<- as.numeric(concordance(smod.new ~ exp(-linear.kkpcox.prediction))[1])
  ...
  for ( q in 1:F){
    ...
    reg <- glmnet(x = Y.tmp, y = time.tmp, family = "gaussian")
    coeff.pred <- coef(object =reg, newx = Y.tmp, s= reg.pcox$lambda[35])
    rel.coeff <- coeff.pred[2:(D+1)] 
    ind.rel <- which(rel.coeff !=0)
    linear.aft[ind] <- predict(object = reg, newx = Y.tmp, s = reg.pcox$lambda[35]) 
  }
  recovCIndex.km.paft <<- as.numeric(concordance(smod ~ linear.aft)[1])
  
  
  
  #### prediction with penAFT ###################################################################
  linear.kkpaft.prediction <- c(0)
  to_iterate<-unique(as.numeric(label.test))
  for ( q in to_iterate){
    ind <- which(label.train == q)
    ind.new <- which(label.test == q)
    reg.aft <- glmnet(x = Y[ind,], y = time[ind], family = "gaussian")
    linear.kkpaft.prediction[ind.new] <- predict(object =reg.aft, newx = as.matrix(Y.new[ind.new,]), s= reg.pcox$lambda[35])
  }
  predCIndex.kn.paft <<- as.numeric(concordance(smod.new ~ linear.kkpaft.prediction)[1])
  ...
  for (q in 1:F){
    ...
    coxreg$status <- censoring.tmp
    reg.pcox <- glmnet(x = Y.tmp, y = Surv(coxreg$time, coxreg$status), family = "cox")
    linear.pred[ind] <- predict(object =reg.pcox, newx = as.matrix(Y.tmp), s= reg.pcox$lambda[35])
    if (length(ind.new)==1){
      cox.pred[ind.new] <- predict(object =reg.pcox, newx = as.matrix(t(Y.tmp.new)), s= reg.pcox$lambda[35])
    }else{ 
    cox.pred[ind.new] <- predict(object =reg.pcox, newx = as.matrix(Y.tmp.new), s= reg.pcox$lambda[35])
      } 
    }

  recovCIndex.true.pcox <<- as.numeric(concordance(smod ~ exp(-linear.pred))[1])
  ####ATTENTION THIS LINE CAMILA HAS COMMENTED BECAUSE IS NOT WORKING YET####
  #predCIndex.true.knn.pcox <<- as.numeric(concordance(smod.new ~ exp(-cox.pred))[1])
  ...
  # }
  # recovCIndex.true.paft <<- as.numeric(concordance(smod ~ exp(-linear.aft))[1])
  # predCIndex.true.knn.paft <<- as.numeric(concordance(smod.new ~ exp(-pred.aft))[1])
  # 
  # 
