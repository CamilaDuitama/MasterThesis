...
```{r LibrariesLoading, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
library(sva)
library(readr)
library(ggplot2)
library(matrixcalc)
...
```{r Loading files, echo=FALSE, message=TRUE, warning=FALSE, paged.print=FALSE}
######## Input ###############
...
```{r Number of samples, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE, eval= FALSE}
...
```{r PCA of the whole mRNA dataset, echo=FALSE}
...
p8 <- ggplot(as.data.frame(pc.unscaled$x), aes(x=pc.unscaled$x[,1], y= pc.unscaled$x[,2], colour= as.factor(labels))) + ggtitle("Unscaled data") + geom_point(shape=19) + labs(y = "PC1", x = "PC2", colour = "Classes") 
p8
```


```{r Testing, echo=FALSE, message=FALSE, warning=FALSE, paged.print=FALSE}
smod.new <- Surv(time.new, censoring.new)
