...
### mRNA headers of the matrix with  GSVA scores
...
p1 <- ggplot(mRNA.pca, aes(x=PC1, y=PC2, colour=Classes)) + ggtitle(" PCA of the training and testing set \n GSVA Scores of the Canonical Pathway Sets") + geom_point(shape=19) + labs(y = "PC1", x = "PC2", colour = "Classes") 
p1
...
p6 <- ggplot(mRNA.signature.pca, aes(x=PC1, y=PC2, colour=Classes)) + ggtitle(" PCA of the training and testing set \n SBC Signature of the GSVA Scores on the Canonical Pathways Gene Sets") + geom_point(shape=19) + labs(y = "PC1", x = "PC2", colour = "Classes") 
p6
```
...
```{r Consolidated clustering results for the training set}
table(c.sbc) %>%
  kable(caption = "Clusters for the training set") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed","responsive"),full_width = T, position = "center")
```

```{r Consolidated clustering results for the test set}
table(c.sbc.new) %>%
