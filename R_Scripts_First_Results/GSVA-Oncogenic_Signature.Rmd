...
subtitle: "GSVA-Oncogene set preprocessing"
...
params:
  ...
  iter.thin: 5
  pval.sbc : 0.95
  output_folder : "GSVA-Oncogenic_Signature_files/"
---
...
load("/Volumes/GoogleDrive/My Drive/Documents/GitHub/MasterThesis/R_Scripts_First_Results/GSVA-Oncogenic_Signature.RData")

```

# Description

The Oncogenic Gene Sets were downloaded from [MSigDB](http://software.broadinstitute.org/gsea/msigdb/genesets.jsp?collection=C6) and a Gene Set Enrichment Score was calculated for each of the methods present in the bioconductor package GSVA : ssGSEA, Zscore, PLAGE and GSVA. The method with the lowest P-values was selected as pathway feature aggregation method to give the scores that would be used as inputs for the SBC model.
...
```{r Selection of gene set scoring method ,eval=FALSE}
gs<-list()
...
p8 <- ggplot(as.data.frame(pc.unscaled$x), aes(x=pc.unscaled$x[,1], y= pc.unscaled$x[,2], colour= as.factor(labels))) + ggtitle("PCA \n Training set with SBC labels and unlabeled test set") + geom_point(shape=19) + labs(y = "PC1", x = "PC2", colour = "Classes") +scale_color_manual(values=c("#b16ed3","#d37f6e","#90d36e","#6ec2d3","#858687"))
p8
```
