...
params:
  ...
  iter.thin: 5
  pval.sbc : 0.95
  output_folder : "OcogenicGeneSets+Pcox_files/"
output:
  html_notebook:
    highlight: tango
    toc: yes
...
```
# Oncogene set Pcox fitting of model
## Training set
...
for (i in unique(db_genesets$gs_name)){
  ...
  reg.pcox[[i]] <- glmnet(x = as.matrix(df), y = smod, family = "cox")
  lp[[i]] <- predict(object =reg.pcox[[i]], newx = as.matrix(df), s=min(reg.pcox[[i]]$lambda) ,type="link")
}
...
Y.pre.train<-do.call(cbind,lp)
colnames(Y.pre.train)<-names(lp)
...
for (i in unique(db_genesets$gs_name)){
  ...
  reg.pcox.new[[i]] <- glmnet(x = as.matrix(df.new), y = smod.new, family = "cox")
  lp.new[[i]] <- predict(object =reg.pcox.new[[i]], newx = as.matrix(df.new), s=min(reg.pcox.new[[i]]$lambda) ,type="link")
}
toplot.new<-as.data.frame(x=unlist(lost.new))
colnames(toplot.new)<-"Percentage"
ggplot(toplot.new, aes(x="", y=Percentage)) + 
    geom_boxplot(fill="slateblue", alpha=0.2) + labs(title = "Boxplot from percentage of lost ids per gene set", subtitle="Testing Set")

Y.pre.test<-do.call(cbind,lp.new)
colnames(Y.pre.test)<-names(lp)
```
...
```{r PCA of the signature, include=FALSE}
...
p8 <- ggplot(as.data.frame(pc.unscaled$x), aes(x=pc.unscaled$x[,1], y= pc.unscaled$x[,2], colour= as.factor(labels))) + ggtitle("Scaled data") + geom_point(shape=19) + labs(y = "PC1", x = "PC2", colour = "Classes")+scale_color_manual(values=c("#b16ed3","#d37f6e","#90d36e","#6ec2d3","#C70039"))
p8
```
