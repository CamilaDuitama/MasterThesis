...
ComparisionFLX = function(){
  ...
  recovCIndex.flx <<- as.numeric(concordance(smod ~ linear.recov.flx)[1])
  ...
  if (F>gr.flx@k){
    for ( q in 1:gr.flx@k){
      ...
      lambda<-reg$lambda[round(length(reg$lambda)/2)]
      
      coeff.pred <- coef(object =reg, newx = Y.tmp, s= lambda)
      rel.coeff <- coeff.pred[2:(D+1)] 
      beta.flx[1:D,q] <- ((rel.coeff != 0)+0)
      
      linear.flx[ind] <- predict(object = reg, newx = Y.tmp, s = lambda) }
    }else{
      for ( q in 1:F){
        ind <- which(clusters(gr.flx) == q)
        L= length(ind)
      ...
      recovCIndex.flx.aft  <<- as.numeric(concordance(smod ~ linear.flx)[1])
      ...
      predCIndex.flx <<- as.numeric(concordance(smod.new ~ linear.pred.flx)[1])
      ...
      # }
      # predCIndex.flx.aft  <<- as.numeric(concordance(smod.new ~ exp(-pre.flx))[1])
      # 
      # 
