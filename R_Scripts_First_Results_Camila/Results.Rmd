...
```{r LibrariesLoading, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
library(sva)
library(readr)
library(ggplot2)
library(matrixcalc)
...
```{r Loading files, echo=TRUE, warning=FALSE, paged.print=FALSE}
######## Input ###############
...
```{r SBC Signature, echo=TRUE, message=TRUE, warning=TRUE, paged.print=FALSE}
...
```{r Training of SBC, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
##############################################################################
...
```{r PCA Unscaled Y and Y.new, echo=TRUE}
...
for (i in 1:Nps){
  if (length(table(c.matrix.new[,i]))==4 & lr[i]<0.05){
    c.sbc.new<-c.matrix.new[,i]
  }
}

if (length(c.sbc.new)==0){
