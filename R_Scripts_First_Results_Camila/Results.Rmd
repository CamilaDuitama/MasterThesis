...
```{r Loading files, eval=FALSE, message=TRUE, warning=FALSE, include=FALSE, paged.print=FALSE, echo=TRUE}
######## Input ###############
...
#Code to remove the indexes where both the time and censoring info is NaN
to_remove<-which(is.na(Clinical_TestSet$days_to_death) & is.na(Clinical_TestSet$days_to_last_followup), arr.ind=TRUE)
Clinical_TestSet<-Clinical_TestSet[-(to_remove),]
mRNAArray_TestSet<-mRNAArray_TestSet[-(to_remove),]
...
```{r PCA of the whole mRNA dataset, message=FALSE, warning=FALSE, paged.print=FALSE}
totalmRNA<-bind_rows(mRNAArray_TrainingSet,mRNAArray_TestSet)
...
```
```{r PCA miRNA, eval=FALSE, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
...
miRNA_TestSet <- read_csv("/Volumes/GoogleDrive/My Drive/Documents/Life Science Informatics Master/Master thesis/Data/First_data_split/TestSet/miRNA_TestSet.csv", 
    col_types = cols(X1 = col_skip()))
totalmiRNA<-bind_rows(miRNA_TrainingSet,miRNA_TestSet)
miRNA.pca<-as.data.frame(prcomp(totalmiRNA,scale=TRUE, center= TRUE)$x[,1:2])
...
```{r Testing, echo=TRUE, message=FALSE, warning=FALSE, paged.print=FALSE}
...
Y.big<-rbind(Y,Y.new)
pc <- prcomp(Y.big, scale=TRUE, center= TRUE)
labels<-c.sbc
for (i in 1:dim(Y.new)[1]){labels<-append(labels,5)}
p8 <- ggplot(as.data.frame(pc$x), aes(x=pc$x[,1], y= pc$x[,2], colour= as.factor(labels))) + ggtitle(" SBC Clustering \n VerhaakCancer Data Set") + geom_point(shape=19) + labs(y = "PC1", x = "PC2", colour = "Classes") 
p8
...
pc.unscaled <- prcomp(Y.big.unscaled, scale=TRUE, center= TRUE)
p9 <- ggplot(as.data.frame(pc.unscaled$x), aes(x=pc.unscaled$x[,1], y= pc.unscaled$x[,2], colour= as.factor(labels))) + ggtitle(" SBC Clustering \n VerhaakCancer Data Set") + geom_point(shape=19) + labs(y = "PC1", x = "PC2", colour = "Classes") 
p9
```


```{r Testin2, eval=FALSE, include=FALSE}
