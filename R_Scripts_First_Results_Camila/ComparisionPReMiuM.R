...
ComparisionPReMiuM = function(){
  ...
  recovCIndex.premium.aft  <<- as.numeric(concordance(smod ~ exp(-linear.premium))[1])
  predCIndex.premium.aft  <<- as.numeric(concordance(smod.new ~ exp(-linear.premium.pred))[1])
  ...
  predCIndex.premium <<-  as.numeric(concordance(smod.new ~ exp(-output_predictions$predictedY))[1])
